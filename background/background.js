...
function getElapsedTime() {
	...
	chrome.storage.local.get(
		["LastRun", "goodreadsID", "overdriveURL", "error", "count"],
...
function queryGoodreads(goodreadsID, overdriveURL) {
	...
	let carousel_messages = [
		"We'll scan the most recent 200 books on your Goodreads To-Read shelf to find titles already available at your local OverDrive library.",
		"We'll automatically refresh your library every 24 hours so titles are always up-to-date.",
	...
	fetch(
		"https://www.goodreads.com/review/list?v=2&id=" +
			goodreadsID +
			"&shelf=to-read&sort=position&order=d&per_page=200&key=" +
			apiKeys.goodreads,
		{
			method: "GET",
			headers: {
		...
		.then(function (data) {
			...
			for (let i = 0; i < bookArray.length; i++) {
				ToRead.push({
					title: bookArray[i].book.title,
					author: bookArray[i].book.authors.author.name,
				});
			}
	...
	for (let i = 0; i < ToRead.length && currently_scanning; i++) {
		await fetchWithTimeout(
			...
			.then(function (html) {
				...
				let overdriveSplicedSearchResults = overdriveSearchResults.slice(
					30,
					overdriveSearchResults.length - 1
				);
				...
				if (JSON_length === 0) {
					console.log(
						ToRead[i].title + " by " + ToRead[i].author + " not found."
					);
				}
							...
							if (
								...
								for (let i = 0; i < BookAvailability.length; i++) {
									if (BookAvailability[i].type === "eBook" && ebookToggle) {
										...
										BookAvailability[i].type === "Audiobook" &&
										audiobookToggle
									) {
										if (BookAvailability[i].available === true) {
											Available.push(BookAvailability[i]);
