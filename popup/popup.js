...
document.addEventListener("DOMContentLoaded", function () {
	...
	document
		.getElementById("audiobook_toggle")
		.addEventListener("click", toggleType);
	document
		.getElementById("ebook_toggle_settings")
		.addEventListener("click", toggleType);
	document
		.getElementById("audiobook_toggle_settings")
		.addEventListener("click", toggleType);
});
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	if (request.msg === "In Progress") {
		...
		if (
			$("#loading_carousel_message").html() !=
			"<em>" + request.text + "</em>"
		) {
			$("#loading_carousel_message")
		...
		$("#loading_carousel_message").html(
			"<em>We'll scan the most recent 200 books on your Goodreads To-Read shelf to find titles already available at your local OverDrive library.</em>"
		);
		...
		async function (result) {
			...
			if (
				...
				// else no errors, proceed to show library
					...
					document.getElementById("ebook_toggle_settings").checked =
						ebookToggle;
					document.getElementById("audiobook_toggle").checked = audiobookToggle;
					document.getElementById("audiobook_toggle_settings").checked =
						audiobookToggle;
...
function updateMainPage(BookAvailability) {
	...
	if (Available.length == 0 && Holds.length == 0) {
		document.getElementById("available_now").innerHTML +=
			"<div class='d-flex flex-column justify-content-center text-center'><h1 class='mt-5 mb-3'><i class='fa fa-lg fa-frown-o' aria-hidden='true'></i></h1><h2>Sorry...</h2><h3>No titles were found.</h3></div>";
	}
	// else, books exist and are ready for display
		...
		for (let i = 0; i < Available.length; i++) {
			document.getElementById("available_now").innerHTML +=
				"<div class='d-flex border-bottom pb-2 mb-2'><img src='" +
				Available[i].cover +
				"' class='cover' alt='" +
				Available[i].title +
				"'><div class='d-flex flex-column justify-content-center overflow-hidden w-75 ml-3 mr-3'><h6 class='text-nowrap text-truncate mb-0'>" +
				Available[i].title +
				"</h6><p class='text-nowrap text-truncate mb-0'>" +
				Available[i].author +
				"</p><p class='text-nowrap text-truncate mb-3'><span class='mr-1'>" +
				(Available[i].type == "eBook" ? "📘" : "🎧") +
				"</span><em>" +
				Available[i].type +
		...
		if (Available.length > 0) {
			document.getElementById("available_now").innerHTML +=
				"<div class='d-flex align-items-center border-bottom pt-4 pb-3 pb-4'><h5 class='p-2 m-0'>Find a good book?</h5><a type='button' class='btn btn-success' href='https://chrome.google.com/webstore/detail/mfckggnkebdpaocogfekaaicafooeiik/review' target='_blank'>Leave a review!</a></div>";
		}
		...
		for (let i = 0; i < Holds.length; i++) {
			document.getElementById("available_soon").innerHTML +=
				...
				Holds[i].type +
				"</em><br>~" +
				(Holds[i].estimatedWait != undefined
					? Holds[i].estimatedWait >= 14
						? Math.round(Holds[i].estimatedWait / 7) + " week wait"
						: Holds[i].estimatedWait + " day wait"
					: "Unknown wait") +
				"</em></p><a type='button' class='btn btn-outline-secondary hold' href='" +
				Holds[i].URL +
				"' target='_blank'>Place Hold</a></div></div>";
		}
	...
	document.getElementById("available_count").innerText =
...
function saveUserData(e) {
	...
	chrome.storage.local.set({
		goodreadsID: goodreadsID,
		overdriveURL: overdriveURL,
		ebook_toggle: document.getElementById("ebook_toggle").checked,
		audiobook_toggle: document.getElementById("audiobook_toggle").checked,
	});
